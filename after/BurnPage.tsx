...
import useTokenSupplyStats from './useTokenSupplyStats';

const BurnPageStyled = styled.div``;

export const BurnPage = () => {
  const { walletChain, isOldToken } = useWallet();
  const { openConnectModal } = useChainSelector();
  const { burnAmount, txButton, txProgress, burnTxHash, executeBurn, onChangeBurnAmount } = useTokenBurn(walletChain);
  const { coinData } = useTokenSupplyStats(walletChain?.id, isOldToken);
